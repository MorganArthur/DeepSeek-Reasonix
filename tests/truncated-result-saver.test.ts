...
describe("saveTruncatedResult", () => {
  ...
  it("writes content to .xiaomi-reasonix/truncated-results/", () => {
    ...
    expect(path).toMatch(/^\.xiaomi-reasonix\/truncated-results\/\d+-[a-f0-9]+-web_search\.txt$/);
  ...
  it("creates the directory if missing", () => {
    ...
    expect(existsSync(join(rootDir, ".xiaomi-reasonix", "truncated-results"))).toBe(true);
  ...
  it("falls back to ~/.xiaomi-reasonix when rootDir is empty", () => {
    ...
    try {
      ...
      expect(path).toMatch(/\.xiaomi-reasonix\/truncated-results\//);
...
describe("cleanupOldResults", () => {
  ...
  it("removes files older than maxAgeMs", () => {
    const dir = join(rootDir, ".xiaomi-reasonix", "truncated-results");
