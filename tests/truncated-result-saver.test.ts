...
describe("saveTruncatedResult", () => {
  ...
  it("writes content to .reasonix-xiaomi/truncated-results/", () => {
    ...
    expect(path).toMatch(/^\.reasonix-xiaomi\/truncated-results\/\d+-[a-f0-9]+-web_search\.txt$/);
  ...
  it("creates the directory if missing", () => {
    ...
    expect(existsSync(join(rootDir, ".reasonix-xiaomi", "truncated-results"))).toBe(true);
  ...
  it("falls back to ~/.reasonix-xiaomi when rootDir is empty", () => {
    ...
    try {
      ...
      expect(path).toMatch(/\.reasonix-xiaomi\/truncated-results\//);
...
describe("cleanupOldResults", () => {
  ...
  it("removes files older than maxAgeMs", () => {
    const dir = join(rootDir, ".reasonix-xiaomi", "truncated-results");
