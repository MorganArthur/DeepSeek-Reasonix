/** `~/.reasonix-xiaomi/memory/` store + prefix-loading composer — temp homeDir per test. */
...
describe("user-memory", () => {
  ...
  describe("applyMemoryStack", () => {
    it("composes REASONIX.md → global memory → project memory", () => {
      ...
      // applyMemoryStack uses ~/.reasonix-xiaomi by default — redirect via HOME
    ...
    it("applyMemoryStack injects no memory blocks when no memory is set", () => {
      ...
      // dev's real ~/.reasonix-xiaomi and bleeds in whatever memory they have.
  ...
  describe("applyGlobalReasonixMemory", () => {
    it("loads ~/.reasonix-xiaomi/REASONIX.md when present", () => {
