...
export const zhCN: TranslationSchema = {
  ...
  sessions: {
    ...
    listHeader: "保存的会话 (~/.reasonix-xiaomi/sessions/)：",
  ...
  ui: {
    ...
    noConfigHint: "本次运行忽略 ~/.reasonix-xiaomi/config.json",
  ...
  slash: {
    ...
    memory: {
      description: "显示 / 管理固定记忆（REASONIX.md + ~/.reasonix-xiaomi/memory）",
    ...
    hooks: {
      description:
        "列出活跃的 hooks（.reasonix-xiaomi/ 下的 settings.json）· reload 从磁盘重新读取",
      argsHint: "[reload]",
    },
    permissions: {
      description:
        "显示 / 编辑 shell 允许列表（内置只读 · 项目级：~/.reasonix-xiaomi/config.json）",
  ...
  handlers: {
    basic: {
      ...
      helpMemoryGlobal:
        "  #g <note>                将 <note> 追加到 ~/.reasonix-xiaomi/REASONIX.md（全局，不提交）。",
    ...
    admin: {
      ...
      hooksProjectFallback: "  · <project>/.reasonix-xiaomi/settings.json（项目）",
      hooksGlobal: "  · {path}（全局）",
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear 仅在 `reasonix code` 中可用 — 它们编辑项目范围的允许列表（`~/.reasonix-xiaomi/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    memory: {
      disabled:
        "记忆已禁用（环境变量 REASONIX_MEMORY=off）。取消设置该变量以重新启用 — 此期间不会固定任何 REASONIX.md 或 ~/.reasonix-xiaomi/memory 内容。",
      noRoot:
        "此会话无工作目录 — `/memory` 需要一个根目录来解析 REASONIX.md。（在测试环境中运行？）",
      listEmpty:
        "尚无用户记忆。模型可以调用 `remember` 保存一个，或您可以在 ~/.reasonix-xiaomi/memory/global/ 或项目子目录中手动创建文件。",
      ...
      layerGlobal: "  2. ~/.reasonix-xiaomi/memory/global/ — 您的跨项目私有记忆。",
      layerProjectHash: "  3. ~/.reasonix-xiaomi/memory/<project-hash>/ — 此项目的私有记忆。",
    ...
    skill: {
      ...
      listProjectScope:
        "  · <project>/.reasonix-xiaomi/skills/<name>/SKILL.md  （或 <name>.md） — 项目范围",
      listGlobalScope: "  · ~/.reasonix-xiaomi/skills/<name>/SKILL.md  （或 <name>.md） — 全局范围",
  ...
  pathConfirm: {
    ...
    allowAlways: "始终允许",
    allowAlwaysDesc: "记住 `{prefix}`，本项目永久允许（写入 ~/.reasonix-xiaomi/config.json）",
    deny: "拒绝",
  ...
  webErrors: {
    ...
    tavilyMissingKey:
      "web_search: Tavily 后端需要 API 密钥 — 设置 TAVILY_API_KEY 环境变量，或在 ~/.reasonix-xiaomi/config.json 中配置 `tavilyApiKey`；https://tavily.com 每月 1000 次免费",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: Perplexity 后端需要 API 密钥 — 设置 PERPLEXITY_API_KEY 环境变量，或在 ~/.reasonix-xiaomi/config.json 中配置 `perplexityApiKey`；在 https://perplexity.ai/settings/api 获取密钥",
    ...
    exaMissingKey:
      "web_search: Exa 后端需要 API 密钥 — 设置 EXA_API_KEY 环境变量，或在 ~/.reasonix-xiaomi/config.json 中配置 `exaApiKey`；https://exa.ai 每月 1000 次免费",
    exaUnauthorized:
