...
export const de: TranslationSchema = {
  ...
  sessions: {
    ...
    listHeader: "Gespeicherte Sitzungen (~/.reasonix-xiaomi/sessions/):",
  ...
  ui: {
    ...
    noConfigHint: "Ignoriere bei diesem Durchlauf die Datei ~/.reasonix-xiaomi/config.json",
  ...
  slash: {
    ...
    memory: {
      ...
      description: "Pinned Memory anzeigen / verwalten (REASONIX.md + ~/.reasonix-xiaomi/memory)",
    },
    skill: {
      ...EN.slash.skill,
      description:
    ...
    hooks: {
      ...
      argsHint: "[Neu laden]",
      description:
        "Aktive Hooks auflisten (settings.json unter .reasonix-xiaomi/) · reload liest von Platte neu",
    },
    permissions: {
      ...
      description:
        "Shell-Allowlist anzeigen / bearbeiten (builtin schreibgeschützt · pro Projekt: ~/.reasonix-xiaomi/config.json)",
    },
    dashboard: {
      ...EN.slash.dashboard,
      argsHint: "[Stopp]",
  ...
  handlers: {
    ...
    basic: {
      ...
      helpMemoryGlobal:
        "  #g <notiz>                <notiz> an ~/.reasonix-xiaomi/REASONIX.md anhängen (global, niemals committed).",
      helpMemoryGlobalEx:
    ...
    admin: {
      ...
      hooksProjectFallback: "  · <projekt>/.reasonix-xiaomi/settings.json (Projekt)",
    ...
    permissions: {
      ...
      mutateCodeOnly:
        "/permissions add / remove / clear sind nur innerhalb von `reasonix code` verfügbar — sie bearbeiten die projektbezogene Allowlist (`~/.reasonix-xiaomi/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    memory: {
      ...EN.handlers.memory,
      disabled:
        "Memory ist deaktiviert (REASONIX_MEMORY=off in der Umgebung). Entferne die Variable zur Reaktivierung — es werden weder REASONIX.md noch ~/.reasonix-xiaomi/memory-Inhalte eingefügt.",
      noRoot:
      ...
      listEmpty:
        "Noch keine Benutzer-Memories. Das Modell kann `remember` aufrufen, um einen zu speichern, oder du kannst Dateien manuell in ~/.reasonix-xiaomi/memory/global/ oder dem projektspezifischen Unterverzeichnis erstellen.",
      ...
      layerGlobal:
        "  2. ~/.reasonix-xiaomi/memory/global/ — dein projektübergreifendes privates Memory.",
      layerProjectHash:
        "  3. ~/.reasonix-xiaomi/memory/<projekt-hash>/ — privates Memory dieses Projekts.",
    ...
    skill: {
      ...
      listProjectScope:
        "  · <projekt>/.reasonix-xiaomi/skills/<name>/SKILL.md  (oder <name>.md)  — Projekt-Bereich",
      listGlobalScope:
        "  · ~/.reasonix-xiaomi/skills/<name>/SKILL.md  (oder <name>.md)  — globaler Bereich",
  ...
  pathConfirm: {
    ...
    allowAlwaysDesc:
      "`{prefix}` für dieses Projekt merken (gespeichert in ~/.reasonix-xiaomi/config.json)",
    deny: "ablehnen",
  ...
  webErrors: {
    ...
    tavilyMissingKey:
      "web_search: Tavily-Backend benötigt einen API-Schlüssel — setze TAVILY_API_KEY-Umgebungsvariable oder `tavilyApiKey` in ~/.reasonix-xiaomi/config.json; kostenlose 1000/Monat-Registrierung unter https://tavily.com",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: Perplexity-Backend benötigt einen API-Schlüssel — setze PERPLEXITY_API_KEY-Umgebungsvariable oder `perplexityApiKey` in ~/.reasonix-xiaomi/config.json; erhalte einen unter https://perplexity.ai/settings/api",
    ...
    exaMissingKey:
      "web_search: Exa-Backend benötigt einen API-Schlüssel — setze EXA_API_KEY-Umgebungsvariable oder `exaApiKey` in ~/.reasonix-xiaomi/config.json; kostenlose 1000/Monat-Registrierung unter https://exa.ai",
    exaUnauthorized:
    ...
    braveMissingKey:
      "web_search: Für Brave Search ist ein API-Schlüssel erforderlich — setze die Umgebungsvariable BRAVE_SEARCH_API_KEY (oder BRAVE_API_KEY) oder `braveApiKey` in ~/.reasonix-xiaomi/config.json; kostenlose Anmeldung mit 2000 Einheiten pro Monat unter https://brave.com/search/api/",
    braveUnauthorized:
