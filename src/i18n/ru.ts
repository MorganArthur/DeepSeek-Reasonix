...
export const ru: TranslationSchema = {
  ...
  sessions: {
    ...
    listHeader: "Сохранённые сессии (~/.reasonix-xiaomi/sessions/):",
  ...
  ui: {
    ...
    noConfigHint: "игнорировать ~/.reasonix-xiaomi/config.json для этого запуска",
  ...
  slash: {
    ...
    memory: {
      ...EN.slash.memory,
      description:
        "показать / управлять закреплённой памятью (REASONIX.md + ~/.reasonix-xiaomi/memory)",
    ...
    hooks: {
      ...EN.slash.hooks,
      description:
        "список активных хуков (settings.json в .reasonix-xiaomi/) · reload перечитывает с диска",
    },
    permissions: {
      ...EN.slash.permissions,
      description:
        "показать / редактировать белый список команд (встроенные только для чтения · на проект: ~/.reasonix-xiaomi/config.json)",
    },
    dashboard: {
      ...EN.slash.dashboard,
  ...
  webErrors: {
    ...EN.webErrors,
    braveMissingKey:
      "web_search: Для работы Brave Search требуется ключ API — задайте переменную среды BRAVE_SEARCH_API_KEY (или BRAVE_API_KEY) или параметр `braveApiKey` в файле ~/.reasonix-xiaomi/config.json; бесплатная регистрация с лимитом 2000 запросов в месяц на сайте https://brave.com/search/api/",
    braveUnauthorized:
