...
export interface CodeToolsetOpts {
  rootDir: string;
  /** Override the default `~/.xiaomi-reasonix/config.json` lookup — primarily for tests that pin a tmp config. */
  configPath?: string;
...
export async function buildCodeToolset(opts: CodeToolsetOpts): Promise<CodeToolset> {
  ...
  // Lazy: constructing DeepSeekClient throws when XIAOMI_API_KEY is unset,
