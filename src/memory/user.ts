...
export interface MemoryStoreOptions {
  /** Override `~/.reasonix-xiaomi` — tests set this to a tmpdir. */
  homeDir?: string;
...
export class MemoryStore {
  ...
  constructor(opts: MemoryStoreOptions = {}) {
    this.homeDir = opts.homeDir ?? join(homedir(), ".reasonix-xiaomi");
...
export function readGlobalReasonixMemory(
  homeDir: string = join(homedir(), ".reasonix-xiaomi"),
...
export function applyGlobalReasonixMemory(basePrompt: string, homeDir?: string): string {
  ...
  const dir = homeDir ?? join(homedir(), ".reasonix-xiaomi");
  const mem = readGlobalReasonixMemory(dir);
  if (!mem) return basePrompt;
  return [
    basePrompt,
    "",
    "# Global memory (~/.reasonix-xiaomi/REASONIX.md)",
    "",
...
export function applyUserMemory(
  ...
  if (global) {
    parts.push(
      "",
      "# User memory — global (~/.reasonix-xiaomi/memory/global/MEMORY.md)",
      "",
...
export function applyMemoryStack(
  ...
  const withGlobal = applyGlobalReasonixMemory(
    withProject,
    homeDir ? join(homeDir, ".reasonix-xiaomi") : undefined,
  );
