...
const CONFIG_TABS = [
  {
    ...
    files: [
      {
        name: '~/.reasonix-xiaomi/config.json',
        lang: 'json',
        code: `{
  ...
  },
  {
    ...
    bullets: [
      { zh: '项目级: <project>/.reasonix-xiaomi/skills/<name>.md', en: 'Project: <project>/.reasonix-xiaomi/skills/<name>.md' },
      { zh: '全局: ~/.reasonix-xiaomi/skills/<name>.md', en: 'Global: ~/.reasonix-xiaomi/skills/<name>.md' },
    ...
    files: [
      {
        name: '.reasonix-xiaomi/skills/review-pr.md',
        lang: 'md',
        code: `---
...
› please review the current branch`,
  ...
  },
  {
    ...
    desc: {
      zh: 'Reasonix 把"应当记住"的内容拆成两层：仓库级的 reasonix.md（提交进 git，团队共享）与用户级的 ~/.reasonix-xiaomi/memory.md（个人偏好，不入库）。每次会话启动时自动注入到 prompt 头部。',
      en: 'Reasonix splits "what to remember" into two layers: repo-level `reasonix.md` (checked into git, shared with the team) and user-level `~/.reasonix-xiaomi/memory.md` (personal preferences, kept private). Both are injected at the top of the prompt on every session.',
    },
    bullets: [
      { zh: '<project>/reasonix.md · 项目约定 · git-tracked', en: '<project>/reasonix.md · project conventions · git-tracked' },
      { zh: '~/.reasonix-xiaomi/memory.md · 用户偏好 · 私有', en: '~/.reasonix-xiaomi/memory.md · user preferences · private' },
...
- Don't bump version numbers in package.json`,
      ...
      {
        name: '~/.reasonix-xiaomi/memory.md',
        lang: 'md',
...
- Small commits · one thing at a time`,
  ...
  },
  {
    ...
    desc: {
      zh: '一份 JSON 配置承载所有可调项。全局放 ~/.reasonix-xiaomi/config.json，每个项目可以再用 <project>/.reasonix-xiaomi/config.json 局部覆盖。',
      en: 'A single JSON file holds every knob. The global one lives at `~/.reasonix-xiaomi/config.json`; any project can override it locally with `<project>/.reasonix-xiaomi/config.json`.',
    },
    bullets: [
  ...
  "telemetry": false
      ...
      {
        name: '<project>/.reasonix-xiaomi/config.json',
        lang: 'json',
        code: `{
