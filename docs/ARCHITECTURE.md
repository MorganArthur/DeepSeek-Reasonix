...
set in `.reasonix-xiaomi/settings.json` under the `model` key. No one-shot
...
├── user-memory.ts          # ~/.reasonix-xiaomi/memory/ store (project + global)
...
├── usage.ts                # ~/.reasonix-xiaomi/usage.jsonl roll-up
