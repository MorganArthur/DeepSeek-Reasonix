...
- API keys live in `~/.reasonix-xiaomi/config.json`. Treat that file like any other credential store.
...
- Hooks (`PreToolUse`, etc.) execute arbitrary shell scripts the user has configured. Audit `.reasonix-xiaomi/settings.json` before running Reasonix in a directory you didn't author.
